...
snooper 是一种新的python debug工具，采用装饰器的方式，将函数运行过程以日志输出到文件或终端；
    使用方法参考下面示例，snoop()主要包含四个参数：output、variable、depth、prefix，分别为输出日志文件、打印全局变量、调用函数打印、日志记录时的前缀；
...
#        return num1 + num2 
#    
#    @pysnooper.snoop(output="./log/debug.log",variables=("self.num1","self.num2","self.sum_v"))
